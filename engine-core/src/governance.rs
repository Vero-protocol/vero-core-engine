...
//!
//! Proposals are stored in a single instance-storage map to keep call costs
//! predictable. A proposal moves through this state machine:
//!
//! ```text
//! Pending ──(threshold approvals, strictly increasing signer order)──> Approved ──(timelock elapsed)──> Executed
//! ```
//!
...
/// Ledgers to wait after threshold approval before execution (~1 hour on Stellar).
...
pub enum GovError {
    ...
    DuplicateSigner = 9,
    DuplicateProposal = 10,
    ThresholdNotMet = 11,
    ArithmeticOverflow = 12,
    InvalidSignerOrder = 13,
}
...
pub fn propose(env: &Env, mut proposal: Proposal) -> u64 {
    ...
    proposals.set(id, (proposal.clone(), unlock_ledger));
    save_proposals(env, &proposals);

    publish_event(env, MOD_GOV | ACT_PROPOSE, id, proposal.action_hash.clone());
    id
}

/// Record one signer approval. Approvals must arrive in strictly increasing
/// signer order (by configured signer index) so that a fixed replay of the
/// same approval set always produces the same audit trail. When quorum is
/// reached, the proposal becomes `Approved` and its timelock starts.
